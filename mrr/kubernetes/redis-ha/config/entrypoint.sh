...
echo "REDIS_HA_CLUSTER_STARTUP_REDIS_MASTER_SERVICE_SERVICE_PORT: ${REDIS_HA_CLUSTER_STARTUP_REDIS_MASTER_SERVICE_SERVICE_PORT}"
...
function launchmaster() {
  ...
  sed -i "s/REDIS_DEFAULT_PASSWORD/${REDIS_DEFAULT_PASSWORD}/" /redis-master/redis-master.conf
  redis-server /redis-master/redis-master.conf --protected-mode no
...
function launchslave() {
  ...
  sed -i "s/%master-ip%/${master}/" /redis-slave/redis-slave.conf
  sed -i "s/%master-port%/6379/" /redis-slave/redis-slave.conf
  sed -i "s/REDIS_DEFAULT_PASSWORD/${REDIS_DEFAULT_PASSWORD}/" /redis-slave/redis-slave.conf
  redis-server /redis-slave/redis-slave.conf --protected-mode no
}
...
function launchredis() {
  ...
  while true; do
    ...
    echo "Trying to connect to sentinel: ${REDIS_HA_CLUSTER_SENTINEL_SERVICE_SERVICE_HOST}:${REDIS_HA_CLUSTER_SENTINEL_SERVICE_SERVICE_PORT}, to retireve master's ip"
    master=$(redis-cli -h ${REDIS_HA_CLUSTER_SENTINEL_SERVICE_SERVICE_HOST} -p ${REDIS_HA_CLUSTER_SENTINEL_SERVICE_SERVICE_PORT} --csv SENTINEL get-master-addr-by-name mymaster | tr ',' ' ' | cut -d' ' -f1)
